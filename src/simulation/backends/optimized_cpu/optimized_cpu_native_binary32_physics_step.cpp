// OptimizedCpu physics step routed to native-binary32 collision kernels.
...
constexpr u32 PhysicsStep2MaxSubsteps = 1000u;
constexpr float MwTimeToSeconds = 0.001f;

float SecondsFromMwTime(u32 schemePeriodMwTime) {
    return static_cast<float>(static_cast<int32_t>(schemePeriodMwTime)) *
           MwTimeToSeconds;
}

u32 ComputeNativeBinary32StepSubstepCount(
...
void CHmsZoneDynamic::PhysicsStep2OptimizedCpuNativeBinary32(
    ...
    for (u32 groupIndex = 0;
        ...
        for (u32 groupCorpusIndex = 0;
             ...
             groupCorpusIndex++) {
            CHmsCorpus *corpus =
                    group->NonStaticCorpusAt(groupCorpusIndex);
            ...
            u32 substeps = ComputeNativeBinary32StepSubstepCount(
                    linearSpeedLength,
                    angularSpeedLength,
                    dt,
                    maxStepDistance);
            if (substeps > PhysicsStep2MaxSubsteps) {
                substeps = PhysicsStep2MaxSubsteps;
            }
